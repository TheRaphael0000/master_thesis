\subsection{Fusions methods \label{sec:rank_lists_fusion}}
...
As an extension to the veto method, an additional desired constraint is to favour top ranked link and as well as penalizing bottom ranked links.
...
\begin{figure}
  ...
  \subcaption{$S^-1(x)$, 500 samples (for a rank list with 500 links) in $\left[S(-c), S(+c)\right]$}
  \label{fig:s_curve_c}
  \includegraphics[width=\linewidth]{img/s_curve_c.png}

  \vspace{0.5cm}

  \subcaption{500 sample of $S^-1(x)$ with $r \cdot N$ samples for $\left[S(-c), S(0)\right]$ and $(1-r) \cdot N$ samples for $\left[S(0), S(c)\right]$}
  \label{fig:s_curve_r}
