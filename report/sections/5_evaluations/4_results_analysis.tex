...
\subsubsection{Rank list links distribution across score}

When dealing with rank list, an interesting way to analyse the results is to inspect the true and false links in an histogram.
Figure~\ref{fig:links_distribution_fusion} show the of true and false in an histogram for St-Jean B.
Figure~\ref{fig:links_distribution_fusion_regression} show the rank list obtain by fusion the retained rank list (ref. Table~\ref{tab:9rl_results_st_jean_B} in annex) using the Linear regression fusion method and Figure~\ref{fig:links_distribution_fusion_z_score} the Z-score fusion of the retained rank list on St-Jean B.

{\color{red} todo}

\begin{figure}
  \caption{Links distribution across score on St-Jean B}
  \label{fig:links_distribution_fusion}

  \subcaption{Linear regression fusion (training St-Jean A)}
  \label{fig:links_distribution_fusion_regression}
  \includegraphics[width=\linewidth]{img/links_distribution_fusion_regression.png}

  \subcaption{Z-Score fusion}
  \label{fig:links_distribution_fusion_z_score}
  \includegraphics[width=\linewidth]{img/links_distribution_fusion_z_score.png}
\end{figure}
