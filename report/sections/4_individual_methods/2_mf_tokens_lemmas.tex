\subsection{MF Tokens and Lemmas \label{sec:tokens_lemmas}}
...
If $n$ is too large to present to the user, a dimensionality reduction with, for example the principal component analysis (PCA) can be helpful.
...
The two plots in Figure~\ref{fig:token_vs_lemma} show the average precision (AP) for the resulting rank lists over $n$ for each distance measure.
...
\subsubsection{Importance of the text size in stylometry \label{sec:importance_of_text_size}}
...
\subsubsection{Influance of the most frequent items restriction \label{sec:influance_mf_restriction}}
...
\subsubsection{Frequent errors with the MF tokens \label{sec:frequent_errors}}
...
Figure~\ref{fig:mf_vector_error} show the feature vector of two frequent errors document pairs (document 48 from Vigny / document 62 from Dumas and document 10 from Maupassant / document 52 from Flaubert), these link appear in the top 20 false links of 3 out 4 rank lists.
...
For example, the most similar true link (ranked 1 using Manhattan distance) in Figure~\ref{fig:mf_vector_first_rl} (documents 30 and 116 from Sand) or the HPrec-th (last continuous correct pair from the top of the list) in Figure~\ref{fig:mf_vector_first_last_rl} (documents 184 and 192 from Barbey), both of these links show a large proportion of overlapping surface, like for the frequent errors vectors.

A counter example would be the least similar false link (ranked last using Manhattan distance) which represent a dissimilar document pair, Figure~\ref{fig:mf_vector_last_rl} showcase this link (document 183 from Stael / document 194 from Regnier).
