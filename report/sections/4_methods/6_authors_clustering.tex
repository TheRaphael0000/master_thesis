...
By discarding clustering with $N$ and clusters $1$, This produce $N - 2$ possible clusterings, each of those can be evaluated using the silhouette score.
...
The idea to find the best clustering is to iterate over the number of clusters from $2$ to $N$, and computing median silhouette score for each sample.
When the median silhouette score is below below 0, the clustering stop.
The results are from the last clustering with a positive silhouette score.
...
This ensure that the distance threshold will be at the right place for any new corpus.
