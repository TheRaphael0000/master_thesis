...
The model can find the optimal cut for a rank list from a new corpus (called testing corpus), by estimating the true link probabilities for each link from the previous examples.
...
The merging procedure can be stopped with one of the two following criteria: When a certain cluster number is reached or when the lowest score for the next merge is above a certain value.
...
Each time, the algorithm is stopped at a different cluster number (a different step).
Though this workaround valid, it still introduces an unnecessary overhead of $O(1 + 2 + 3 + ... + n - 1) = O(\frac{n * (n - 1)}{2}) = O(n^2)$ additional merges, with $n$ equal to the number of documents.
...
The agglomerative clustering algorithm was reimplemented to allow access to the clusters at each step, to avoid this overhead.
...
\begin{example}
  ...
  (b), (c) and (d) shows how to update the distances' matrix depending on the linkage criterion.
