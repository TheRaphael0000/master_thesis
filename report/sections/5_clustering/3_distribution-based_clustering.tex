...
This method is a supervised method since it requires examples for learn the cut, even though the only \textit{learnt} parameter is a fixed real number, the distance threshold.
...
The results evaluation are aggregated by applying the arithmetic mean on the $B^3_{F_1}$ and $r_{diff}$ across all retained rank list.
