...
\begin{table}
  \centering
  \caption{Unsupervised clustering evaluation, $B^3_{F_1}$/$r_{diff}$, Maximal Silhouette ($\alpha = 0$)}
...
\subsubsection{Algorithm tweaking for authorship clustering}
...
\begin{definition}[$\alpha$-Silhouette]
  The $\alpha$-Silhouette target score found by using the maximal Silhouette score as a base line and is ajusted using a parameter called $\alpha$.
  \begin{gather*}
    \begin{aligned}
...
\begin{table}
  \centering
  \caption{Unsupervised clustering evaluation, $B^3_{F_1}$/$r_{diff}$, $\alpha = -0.2$}
