...
Statistiques for each metrics and fusion schemes are resumed  Table~\ref{tab:fusion_stats}, it contains for each metric, the minimal value (min), the average and the standard deviation (Avg$\pm$Std), the maximal value, the argmin and argmax for the metrics (ids of the rank list presented above).
...
The model trained to perform the cut is language independent, since the training is only based on the rank list positions of true and false links and the score obtained at each rank.
The model does not depend on the rank list size since only relative magnitudes are used.
A vizual example of the cluster selection is printed
