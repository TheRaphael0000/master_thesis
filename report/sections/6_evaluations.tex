...
The rank list used are the ones obtained by using the 9 following methods ($n=9$), they represent the best configuration retained for each text representation :
\begin{enumerate}
  \setcounter{enumi}{-1}
  \item 750 MFW tokens with Cosine Distance
  \item 750 MFW tokens with Clark
  \item 750 MFW tokens with Manhattan
  \item 750 MFW tokens with Tanimoto
  \item 3000 MFW tokens 3-grams with Cosine distance
  \item 8000 MFW tokens 4-grams with Cosine distance
  \item 250 MFW POS 2-grams with Cosine distance
  \item 1000 MFW POS 3-grams with Cosine distance
  \item Compression using BZip3 with the CBC distance.
\end{enumerate}
...
The results are graphically presented in Figure~\ref{fig:fusions} and statistiques for each metrics are displayed in Table~\ref{tab:fusion_stats}.

To compare the rank list produced by both the Z-Score fusions and the Regression fusion to the non-fused rank lists, two baselines are used.
One called \textit{Single-Max} which represent the best metrics for each rank list in the fusion and another one called \textit{Single-Mean} which is represented by the mean of each metrics in the fusion.
A sign test between the two baseline and the two fusion methods in Table~\ref{tab:fusion_sign_test}.
...
\begin{table*}
  ...
  \subcaption{Training St-Jean B, Testing St-Jean A}
  \label{tab:fusion_A_B}
  \begin{tabular}{l c c c c}
    \toprule
    Metric
  ...
  \subcaption{Training St-Jean A, Testing St-Jean B}
  \label{tab:fusion_A_B}
  \begin{tabular}{l c c c c}
    \toprule
    Metric
...
Using these distance metrics on these datasets, the sign test tends to indicate that the fusion increase the overall quality of the rank list compared to the best rank list in fusion.
