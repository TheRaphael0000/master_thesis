...
To try to increase the quality of the rank list, the proposed method is to use a combination of multiple rank list using different strategies to form an optimistically better rank list.
...
For example if the word : "\textit{the}" is in the N-MFW and occurs 50 times in a document with 300 words, its relative frequency is $50/300 = 1/6$, the word and the value will be in the feature vector if the frequency is in the top-N MFW.
No clear value of N for the N-MFW is to choose over others, but a value between 50-500 tends to produce good results for the word token text representation~\cite{savoy_text_representation}.
...
A simple explaination of why $n$-grams can be effective is by using $n$-grams of size 3 to 5 in most germanic languages this  can correspond, for example to : the tense of the verbs (i.e. : \textit{ing\_}, \textit{ed\_}), some small common words (\textit{\_the\_}, \textit{\_and\_}, \textit{\_that}), or in French the adverbs (\textit{\_ment}), etc.
...
An alternative version of the $n$-grams algorithm used in this study called In-word $n$-grams, to differenciate it from the classical $n$-grams algorithm, called here Letters $n$-grams, is described in Definition~\ref{def:words_n_grams}.
...
In practice the POS is more detailed, for example instead of just considering the verb, a more detailed POS can be the verb and its tense, the same goes for the other type of POS.
...
To avoid having to problems in merging rank list with different magnitude, the solution opted is to translate the scores into a probability of being a true link.
...
The features used to find the logistic parameters for each link in the rank list are : the log of the relative rank and the the link score.
Since a probabilty of being a true link is desired, the target value for each true link is 1 and 0 when it's a false link.

Once a model per type of rank list is trained, this model can provide for any new same type rank list, probabilies of each links being a true link.
The probability framework can now be used on the rank list.
The fusion result in computing the expected value for each links true link's probability from multiple rank list of differant types.
In this case, the expected value is simple average of the probabilities, since no link weighting is used.

\subsubsection{Soft-veto using S-Curve}

An additional desired is to favor top ranked link and penalize bottom ranked links when fusing rank lists.
